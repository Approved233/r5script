...
global struct RTKTweenStaggerChild_Properties
{
	float delay = 0
}
...
void function AnimatePanel( rtk_behavior self, rtk_panel panel )
{
	...
	for ( int childIndex = 0; childIndex < panel.GetChildCount(); ++childIndex )
	{
		...
		float staggerDelay = selfDelay
		rtk_behavior staggerChild = child.FindBehaviorByTypeName( "TweenStaggerChild" )
		if ( staggerChild != null )
		{
			staggerDelay += staggerChild.PropGetFloat( "delay" )
		}
