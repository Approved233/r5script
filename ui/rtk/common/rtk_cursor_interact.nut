...
global struct RTKCursorInteract_Properties
{
	...
	bool keyCodesEnabled = true

	bool absorbOnMouseWheeled = true
	bool absorbOnKeyCodePressed = true

	bool allowKeyCodeInputModeBypass = false 
	bool allowControllerMouseWheel = false 
...
{
	if ( !self.PropGetBool( "allowControllerMouseWheel" ) && IsControllerModeActive() )
		return false
...
{
	if( code == BUTTON_B || code == KEY_ESCAPE ) 
		return false
	...
	if( ( RTKCursorInteract_InputInteractable( self ) || self.PropGetBool( "allowKeyCodeInputModeBypass" ) ) && keycodes.contains(code) || keycodes.len() == 0 )
	{
